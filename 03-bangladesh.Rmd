...
### Deaths compared on a specific day

```{r, echo=FALSE}

# Summary of deaths till date by country on the specific day

bd_day_indicator = deaths_south_asia %>% filter(country == 'Bangladesh', 
                                                date == max(date)) %>%
  pull(day)
...
sa_summary %>%
  kable(format = "html", caption=paste0("Total deaths compared on day ", bd_day_indicator)) %>%
  kable_styling(position = "center")
