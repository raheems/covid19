...
deaths_south_asia <- df_south_asia %>%
  filter(country %in% n_south_asia$country, cum_deaths >0) %>%
...
ggplot(deaths_south_asia, aes(x = day, y = cum_deaths, color=country)) +
  geom_line(size=1.1) +
  geom_point(data = deaths_south_asia[deaths_south_asia$country == 'Bangladesh' & deaths_south_asia$date==max(deaths_south_asia$date), ], aes(x=day, y=cum_deaths), color='red', size=5) +
  geom_text_repel(size=5, aes(label=country),
                  function(deaths_south_asia) deaths_south_asia[deaths_south_asia$date == max(deaths_south_asia$date), ])+
  theme_minimal(base_size = 14) +
  theme(legend.position = "none") +
  # theme_void() + 
  theme(legend.position="none") +
