...
con <- global %>% filter(country != 'Bangladesh(unoff)')
df <- case_fatality(con)
...
### Case Fatality Rate (elect countries)
...
```{r, global-deaths, fig.width=8, fig.height=6, echo = FALSE, warning=FALSE}
# Cumulative Cases since first identified case

# Deaths for Bangladesh
df_bd <- global %>%
  filter(
    country %in% c('Bangladesh', "Bangladesh(unoff)")
  ) %>%
